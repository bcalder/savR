...
The \textit{savR} function is passed a path to an Illumina HiSeq or MiSeq run, and returns a
...
corrected intensities, quality metrics, tile metrics, and extraction metrics.  \textit{savR}
comes with an example MiSeq data set which can be loaded thusly:
...
(full width pixel size of clusters at half maximum), 90th \%-ile intensity of signal intensity, and a datestamp.
